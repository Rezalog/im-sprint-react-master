...
const walkObject = require("sinon/lib/sinon/util/core/walk-object");
...
_.filter = function (arr, test) {
  // TODO: 여기에 코드를 작성합니다.
  let _arr = [];
  // 1. each 사용 : .each(element) 
  // element : test가 통과된 element
  // -> 각 요소에 test 함수를 사용하여 truthy인 경우의 배열만 리턴
  // 2. 리턴받은 함수를 새 배열로 담음
...
_.uniq = function (arr) {
  ...
  _.each(arr, function(elem){
    if (_.indexOf(newarr, elem) === -1){
      newarr.push(elem);
    }
  })
  return newarr;
  // return _.filter(arr, (el, idx) => _.indexOf(el)===idx)
  // return arr.filter((el, idx) => (_.indexOf(el)===idx))
};
