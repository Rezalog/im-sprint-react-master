...
` SELECT category.name
  FROM 
  user INNER JOIN content ON user.id=content.userId
...
` SELECT content.title, content.body, content.created_at
  FROM
  content INNER JOIN content_category ON content.id=content_category.contentId
  INNER JOIN category ON content_category.categoryId = category.id
...
const PART5_2_10 = 
`   
SELECT user.name AS name, COUNT(content.created_at) AS ContentCount
FROM
user LEFT JOIN content ON user.id=content.userId
GROUP BY user.name;
`;
