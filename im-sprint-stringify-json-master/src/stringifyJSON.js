 ...
 */
 ...
 function stringifyJSON(obj) {
  ...
  if(Array.isArray(obj)){
    let newArr = []; //push 메소드 쓰지 않고 배열의 요소를 stringifyJSON(item)값으로 치환할 때도 구현 가능
    for(let item of obj){
...
console.log(stringifyJSON(studentList));
console.log(stringifyJSON('string'));
console.log(stringifyJSON(6));
console.log(stringifyJSON(undefined));
console.log(stringifyJSON(NaN));
console.log(stringifyJSON(Infinity));
