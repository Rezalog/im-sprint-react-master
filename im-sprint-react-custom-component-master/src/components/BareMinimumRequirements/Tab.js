    ...
    <>
      <div>
        <TabMenu>
          ...
          {
            menuArr.map((el, idx) => {
              return <li onClick={() => selectMenuHandler(idx)} className={`submenu${currentTab===idx ? " focused" : ""}`}>Tab {idx}</li>
              // 1. click을 했을때 index 가져옴
              // 2. index === currentTab ? li의 className을 submenu focused : li의 className을 submenu
            })
          }
        </TabMenu>
        <Desc>
